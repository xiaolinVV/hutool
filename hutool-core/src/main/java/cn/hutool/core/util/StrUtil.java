...
public class StrUtil {
    ...
    public static boolean isBlank(CharSequence str) {
        int length;
        // 字符串为 null 或者长度等于0 则为空字符串
        if ((str == null) || ((length = str.length()) == 0)) {
            return true;
        }
        //遍历字符串，判断只要有一个非空字符，即为非空字符串
    ...
    public static String trim(CharSequence str, int mode) {
        ...
        //起始位置，用于 substring 函数裁剪起始位置
        int start = 0;
        //结束位置，用于 substring 函数裁剪结束位置
        int end = length;

        // 扫描字符串头部，从左边起始位置开始扫描，检测到空字符，则起始位置累加1。直到不存在空字符为止
        if (mode <= 0) {
        ...
        // 扫描字符串尾部，从字符尾部位置开始扫描，检测到空字符，则结束位置递减1。直到不存在空字符为止
        if (mode >= 0) {
        ...
        //如果起始位置大于0 或者 结束位置小于字符串长度，说明存在空字符。则截取起始位置到结束位置的字符串。即裁剪掉空字符部分
    ...
    public static String cleanBlank(CharSequence str) {
        ...
        //字符串长度
        int len = str.length();
        //初始化字符串长度的 StringBuilder
        final StringBuilder sb = new StringBuilder(len);
        //字符串中的非空字符
        char c;
        //遍历字符串，找出字符串中非空字符，依次填充进 StringBuilder
        for (int i = 0; i < len; i++) {
            c = str.charAt(i);
    ...
    public static String format(CharSequence template, Map<?, ?> map) {
        ...
        //文本模板中占位符的值
        String value;
        //遍历参数值对，替换原文本模板字符串中包含 {key} 的占位符对应的值
